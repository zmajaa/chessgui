...
class Piece():
    ...
    def moves_available(self, current_position, directions, distance):
        ...
        for x,y in directions:
            ...
            for step in range(1,distance+1):
                ...
                if self.posible_position(destination) not in model.all_occupied_positions():
                    allowed_moves.append(destination)
                elif self.posible_position(destination) in model.all_positions_occupied_by_color(piece.color):
                    collision=True
                else:
                    allowed_moves.append(destination)
                    collision=True
    ...
    def posible_position(self, destination):
